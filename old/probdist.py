...
def log(x, squelch_zero=False): #squelching zero 
    if type(x) == type(Fraction(1,2)):
#         return np.log2(float(x))
        x_float = float(x)
        if x_float == 0.0 and squelch_zero:
            return 0.0
        return math.log2(x_float) if x_float != 0.0 else -1 * np.inf
    if x == 0.0 and squelch_zero:
        return 0.0
    return np.log2(x)
#     return math.log2(x)
